...
export default function AdminTicketDetailPage({
  ...
  async function handleStatusChange(nextStatus: TicketStatus) {
    if (!ticket || !canUpdateTicket || nextStatus === ticket.status || updatingStatus) return;

    setUpdatingStatus(nextStatus);
    setActionError(null);

    try {
      const updatedTicket = await updateAdminTicketStatus(ticket.id, nextStatus);
      setTicket(updatedTicket);
    } catch (error) {
      setActionError(extractApiError(error, "Failed to update ticket status."));
    } finally {
  ...
  async function handleDeleteTicket() {
    if (!ticket || !canDeleteTicket || isDeleting) return;
    ...
    try {
      ...
      setActionError(extractApiError(error, "Failed to delete ticket."));
      setIsDeleting(false);
    }
  }
          ...
          <div className="space-y-4">
              ...
              <div className="flex flex-col gap-2">
                {statusFlow.map((s) => (
                  <button
                    ...
                    disabled={!canUpdateTicket || updatingStatus !== null || s === ticket.status}
            ...
            {canDeleteTicket && (
              <div className="rounded-2xl border border-red-200 bg-white p-5 shadow-sm">
                <h2 className="mb-2 text-sm font-semibold text-red-900">Delete Ticket</h2>
